...
static int info_cnt;
static void add_edge(int s, int e, int token);
...
int parse_gen_init(){
    ...
    nodes = (parse_node*)calloc(MAX_NODE_NUM,sizeof(parse_node));
...
void parse_gen_rule_add_token(int idx, int token)
{
    ...
    r->length++;
    parse_node *node = &nodes[r->end_status];
    //node->rule = 0;
    ...
    if (edge_idx > 0)
    {
        r->end_status = edges[edge_idx].end;
    }
    else
    {
        node_cnt++;
        add_edge(r->end_status, node_cnt, token);
        r->end_status = node_cnt;
    }
    nodes[r->end_status].priority = r->priority;
    nodes[r->end_status].associative = r->associative;
}

static void add_edge(int s, int e, int token)
{
    edge_cnt++;
    edges[edge_cnt].end = e;
    edges[edge_cnt].token = token;
    edges[edge_cnt].next = nodes[s].first_edge;
    nodes[s].first_edge = edge_cnt;
}
